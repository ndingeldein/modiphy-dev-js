var modiphy = ( function( modiphy, Backbone, _ ) {
	...
	M.SiteRouter = Backbone.Router.extend({
		...
		index: function(){
			...
			if(!App.site.isMobile && $(window).innerWidth() > 767){
		...
		selectPage: function( hash, params ){
			...
			if(params){
				...
				if(params.p || params.id || params.cat){
					...
					if(!params.overlay){
						...
						if( nextPage ){
							...
							console.log(this.pages.normal.selected);

							if(this.pages.normal.selected){
								this.pages.normal.deselect( this.pages.normal.selected );
							}
							this.pages.normal.select( nextPage );
	...
	_.extend( M.Site.prototype, {
		...
		load: function(){
			...
			return $.ajax({
		            ...
		            url: 'http://www.webgallerydisplay.com/lib/cached_gallery_jsonp.php',
		...
		introPageSelected: function(){
			...
			this.$logo.attr('href', this.options.directLink);
